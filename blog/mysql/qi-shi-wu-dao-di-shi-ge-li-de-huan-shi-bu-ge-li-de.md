...
但是，在上一篇文章中，和你分享行锁的时候又提到，一个事务要更新一行，如果刚好有另外一个事务拥有这一行的行锁，它又不能这么超然了，会被锁住，进入等待状态。问题是，既然进入了等待状态，那么等到这个事务自己获取到行锁要更新数据的时候，它读到的值又是什么呢？
...
`id` int(11) NOT NULL,  `k` int(11) DEFAULT NULL,  
...
![图 1 事务 A、B、C 的执行流程](<../../.gitbook/assets/image (150).png>)
...
![图 2 行状态变更图](<../../.gitbook/assets/image (145).png>)

图中虚线框里是同一行数据的 4 个版本，当前最新版本是 V4，k 的值是 22，它是被 transaction id 为 25 的事务更新的，因此它的 row trx\_id 也是 25。
...
![图 3 数据版本可见性规则](<../../.gitbook/assets/image (144).png>)
...
![图 4 事务 A 查询数据逻辑图](<../../.gitbook/assets/image (148).png>)
...
![图 5 事务 B 更新逻辑图](<../../.gitbook/assets/image (149).png>)
...
```
mysql> select k from t where id=1 lock in share mode;mysql> select k from t where id=1 for update;复制代码
```

再往前一步，假设事务 C 不是马上提交的，而是变成了下面的事务 C’，会怎么样呢？

![图 6 事务 A、B、C'的执行流程](<../../.gitbook/assets/image (146).png>)
...
![图 7 事务 B 更新逻辑图（配合事务 C'）](<../../.gitbook/assets/image (151).png>)
...
![图 8 读提交隔离级别下的事务状态图](<../../.gitbook/assets/image (141).png>)
